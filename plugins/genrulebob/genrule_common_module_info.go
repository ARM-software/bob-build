...
func (m *genrulebobCommon) getHostBin(ctx android.ModuleContext) android.OptionalPath {
	...
	hostBinModule := ctx.GetDirectDepProxyWithTag(m.Properties.Host_bin, hostToolBinTag)
...
func (m *genrulebobCommon) getArgs(ctx android.ModuleContext) (args map[string]string, dependents []android.Path) {
	...
	fmt.Fprintln(os.Stderr, ctx.OtherModuleName(ctx.Module()))

	// Add arguments providing information about other modules the current
	// one depends on, accessible via ${module}_out.
	ctx.VisitDirectDepsProxyWithTag(generatedDepTag, func(dep android.ModuleProxy) {
		...
		genruleOutputInfoProvider, ok := android.OtherModuleProvider(ctx, dep, GenruleOutputInfoProvider)
		if ok {
			for _, p := range genruleOutputInfoProvider.Outputs {
				outs = append(outs, p.String())
				dependents = append(dependents, p)
			}
			for _, p := range genruleOutputInfoProvider.ImplicitOutputs {
				dependents = append(dependents, p)
			}
		}

		p, ok := android.OtherModuleProvider(ctx, dep, android.CommonModuleInfoProvider)
		if ok && p.OutputFiles != nil {
			for _, p := range p.OutputFiles.DefaultOutputFiles {
				outs = append(outs, p.String())
				dependents = append(dependents, p)
			}
		}

		args[varName+"_out"] = utils.Join(outs)
...
func (m *genrulebobCommon) getModuleSrcs(ctx android.ModuleContext) (srcs []android.Path) {
	ctx.VisitDirectDepsProxyWithTag(generatedSourceTag, func(dep android.ModuleProxy) {
		outProvider, ok := android.OtherModuleProvider(ctx, dep, GenruleOutputInfoProvider)
		if ok {
			for _, p := range outProvider.Outputs {
				srcs = append(srcs, p)
			}
		}
...
func (m *genrulebobCommon) setupBuildActions(ctx android.ModuleContext) (args map[string]string, implicits []android.Path) {
	...
	android.SetProvider(ctx, GenruleExportInclInfoProvider, GenruleExportInclInfo{ExportIncludes: allIncludeDirs})
...
func (m *genrulebobCommon) setOutputProvider(ctx android.ModuleContext) {
	// output info provider needs `m.inout` filled so call after `createInouts`
	var genruleOutputInfo = GenruleOutputInfo{
		Outputs:         m.outputs().Paths(),
		ImplicitOutputs: m.implicitOutputs().Paths(),
	}

	android.SetProvider(ctx, GenruleOutputInfoProvider, genruleOutputInfo)
}
