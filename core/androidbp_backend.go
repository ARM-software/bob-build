...
func (s *androidBpSingleton) GenerateBuildActions(ctx blueprint.SingletonContext) {
	...
	soongCompatFile := getConfig(ctx).Properties.GetString("android_bp_soong_compat_file")
	if soongCompatFile == "" {
		utils.Die("ANDROID_BP_SOONG_COMPAT_FILE is not set. Run setup_android so the Soong compatibility file can be detected.")
	}
