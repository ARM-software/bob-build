...
func transformCmdAndroidToOld(cmd string, ag *androidGenerateRule) (retCmd *string) {
	...
	if strings.Contains(cmd, "$(location)") {
		toolFilesLength := len(ag.ModuleGenruleCommon.Properties.Tool_files)
		toolDepsLength := len(ag.ModuleGenruleCommon.Properties.Tools)
		if toolDepsLength >= 1 && toolFilesLength >= 1 {
			...
			if strings.Contains(ag.ModuleGenruleCommon.Properties.Tools[0], ":") {
				newCmd = strings.Replace(newCmd, "$(location)", "$(location "+strings.Split(ag.ModuleGenruleCommon.Properties.Tools[0], ":")[0]+")", -1)
			} else {
				newCmd = strings.Replace(newCmd, "$(location)", "$(location "+ag.ModuleGenruleCommon.Properties.Tools[0]+")", -1)
			}
		} else {
			newCmd = strings.Replace(newCmd, "$(location)", "$(location "+ag.ModuleGenruleCommon.Properties.Tool_files[0]+")", -1)
		}
	}
...
func transformToolsAndroidToOld(ag *androidGenerateRule, pgs *ModuleGenerateSource) {
	...
	*/
	...
	matches := locationTagRegex.FindAllStringSubmatch(*ag.ModuleGenruleCommon.Properties.Cmd, -1)

	for _, v := range matches {
		tag := v[1]
		// If the tag refers to a tool inside of tool_files, we can just convert it the old command.
		if contains(ag.ModuleGenruleCommon.Properties.Tool_files, tag) {
			newString := strings.Replace(v[0], "$(location", "${tool", 1)
			newString = strings.Replace(newString, ")", "}", 1)
			newCmd := strings.Replace(*ag.ModuleGenruleCommon.Properties.Cmd, v[0], newString, 1)
			ag.ModuleGenruleCommon.Properties.Cmd = &newCmd
			continue
		}
...
func (g *linuxGenerator) androidGenerateRuleActions(ag *androidGenerateRule, mctx blueprint.ModuleContext) {
	...
	proxyGenerateSource.SimpleName.Properties.Name = ag.ModuleGenruleCommon.Properties.Name
	ag.ModuleGenruleCommon.Properties.Cmd = transformCmdAndroidToOld(*ag.ModuleGenruleCommon.Properties.Cmd, ag)
	transformToolsAndroidToOld(ag, &proxyGenerateSource)
	proxyGenerateSource.ModuleGenerateCommon.Properties.Cmd = ag.ModuleGenruleCommon.Properties.Cmd
	proxyGenerateSource.ModuleGenerateCommon.Properties.Tools = ag.ModuleGenruleCommon.Properties.Tool_files
	proxyGenerateSource.ModuleGenerateCommon.Properties.Generated_deps = append(proxyGenerateSource.ModuleGenerateCommon.Properties.Generated_deps, ag.ModuleGenruleCommon.Properties.Tools...)
	proxyGenerateSource.ModuleGenerateCommon.Properties.Export_gen_include_dirs = ag.ModuleGenruleCommon.Properties.Export_include_dirs
	proxyGenerateSource.ModuleGenerateCommon.Properties.Srcs = ag.ModuleGenruleCommon.Properties.Srcs
	proxyGenerateSource.ModuleGenerateCommon.Properties.Exclude_srcs = ag.ModuleGenruleCommon.Properties.Exclude_srcs
	proxyGenerateSource.ModuleGenerateCommon.Properties.Depfile = ag.ModuleGenruleCommon.Properties.Depfile
	proxyGenerateSource.ModuleGenerateCommon.Properties.ResolveFiles(mctx, g)

	proxyGenerateSource.Properties.Implicit_srcs = ag.ModuleGenruleCommon.Properties.Tool_files
	...
	ag.ModuleGenruleCommon.outs = proxyGenerateSource.ModuleGenerateCommon.outs
}
